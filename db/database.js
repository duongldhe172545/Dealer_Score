...
const DB_PATH = path.join(__dirname, 'dealers.db');
...
function calculateCompleteness(dealer, scores) {
  const fields = ['ten_dl', 'ten_chu', 'sdt', 'dia_chi', 'dealer_type', 'category_stack'];
  let filled = 0;
  let total = fields.length + 9; // 6 basic fields + 9 scores
  ...
  if (scores && scores.length > 0) {
    scores.forEach(s => {
      if (s.response && s.response.trim()) filled++;
    });
  }
...
function createDealer(data) {
  ...
  const insertDealer = db.prepare(`
    INSERT INTO dealers (dealer_id, ten_dl, ten_chu, sdt, dia_chi, dealer_type,
      category_stack, has_install_team, est_team_size, c_score, dealer_tier,
